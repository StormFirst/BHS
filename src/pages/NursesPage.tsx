      ...
      <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
        <div className="overflow-x-auto">
          <table className="w-full min-w-[720px] border-collapse text-left text-sm">
            ...
            <tbody>
              {nurses.map((n) => (
                <tr key={n.id} className="border-b border-slate-100 last:border-b-0">
                  <td className="px-4 py-3 font-medium text-slate-900">{n.nurseId}</td>
                  <td className="px-4 py-3 text-slate-700">
                    {(n.firstName ?? '').trim()} {(n.lastName ?? '').trim()}
                  </td>
                  <td className="px-4 py-3 text-slate-700">{Number.isFinite(n.age) ? n.age : '-'}</td>
                  <td className="px-4 py-3 text-slate-700">{formatDate(n.createdAt)}</td>
        ...
        {!loading && nurses.length === 0 ? (
          <div className="p-4 text-sm text-slate-600">Hali hamshiralar yo'q.</div>
        ) : null}
      </div>
      ...
      {confirmDeleteId ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-md rounded-xl bg-white p-5 shadow-lg">
            ...
            <div className="mt-2 text-sm text-slate-600">Rostdan ham bu hamshiralarni o‘chirmoqchimisiz?</div>
